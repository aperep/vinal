...
def squares_sum_solve(q, c, offset = None): 
  ...
  sums = [summands[0]] # ith element consists of all possible sums of first i summands
  for i in range(1,n):
    sums.append(list(dict.fromkeys(p+q for p in sums[i-1].keys() for q in summands[i].values() if p+q <=c)))
  ...
  def solutions(i, x, remainder):
    if i==0:
      ...
      for xi, si in summands[i].items(): # si = qi*xi*xi
        if remainder - si in sums[i-1].keys():
          x[i]=xi
